...
exports.createMembership = async (req, res) => {
    try {
        ...
        if (req.body.cashbackPercent) membership.cashbackPercent = req.body.cashbackPercent;
...
exports.updateMembership = async (req, res) => {
    try {
        ...
        if (req.body.cashbackPercent) membership.cashbackPercent = req.body.cashbackPercent;
        membership.createdBy = req.user._id;
        const record = await Membership.findByIdAndUpdate(membershipId, { $set: membership }, { new: true });
        if (record) {
            res.status(200).json({
                status: 'success',
