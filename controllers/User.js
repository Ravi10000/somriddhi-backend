...
exports.checkAuthentication = async (req, res) => {
    try {
        ...
        if (!doc) throw new AppError("You are not logged in.", 403);
...
exports.getAllUser = async (req, res) => {
    try {
        ...
        if (usertype === undefined || usertype === '') {
            ...
            user = await UserModel.find({ usertype: usertype }).limit(limit).skip(skip).select("-password");
        }
...
exports.login = async (req, res) => {
    try {
        ...
        if (!user) throw new AppError("Please try to login with correct credentials", 400);

        const passwordCompare = await bcrypt.compare(password, user.password);
        if (!passwordCompare) {
            success = false;
            throw new AppError("Please try to login with correct credentials", 400);
        }
...
exports.updateUser = async (req, res) => {
    ...
    try {
        ...
        // }
        result = await UserModel.findByIdAndUpdate(req.params.id, { $set: newData }, { new: true })
        res.json({
            "status": true,
...
exports.phonelogin = async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            ...
            const passwordCompare = await bcrypt.compare(password, user.password);
            if (!passwordCompare) {
                return res.status(400).json({ "status": false, "message": "Please try to login with correct credentials" });
            }
    ...
    exports.resetPassword = async (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            try {
                ...
                const newData = {};
                if (password) { newData.password = secPass };
...
exports.studentAppliedSchools = async (req, res) => {
    try {
        const docs = await FormResponseModel.find({ student: req.user.id }).populate('schoolId');
